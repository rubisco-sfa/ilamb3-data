...
ids = rasterize(
    zip(gdf.geometry, gdf.index, strict=True),  # Index are raster values
    out_shape=(height, width),
    transform=grid_transform,
    fill=-1,
    dtype=np.int16,
    all_touched=True,  # Include all pixels touched by the polygon, not just centroids
)

# Create an xarray Dataset with the rasterized data and metadata
ds = xr.Dataset(
    data_vars={
        "ids": (("lat", "lon"), ids),
        "labels": (gdf["HUC2"].to_numpy()),
...
# Set lat/lon attributes for CF compliance
ds = set_coord_bounds(ds, "lat")
ds = set_coord_bounds(ds, "lon")
ds = set_lat_attrs(ds)
ds = set_lon_attrs(ds)

ds.to_netcdf("HUC2.nc")
